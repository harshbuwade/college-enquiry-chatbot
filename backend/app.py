...
def get_or_create_user():
    ...
    if not user_id:
        ...
        anonymous_user = User(username=f'guest_{session_id}', email=f'guest_{session_id}@temp.com')
...
def get_history():
    try:
        ...
        queries = Query.query.filter_by(user_id=user_id).order_by(Query.timestamp.desc()).limit(20).all()
        history = [{
            'id': q.id,
            'question': q.question,
            'answer': q.answer,
